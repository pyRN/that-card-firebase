...
export default function Spinner({ oCardInfo }) {
  ...
  const aDocsFetched = useSelector((state) => state.oUserReducer.aDocsFetched);
  const fnDispatch = useDispatch();
  console.log("aDocsFetched", aDocsFetched);

  const iRegStart = aDocsFetched.filter(
    (oOwnedInfo) => oOwnedInfo.sId === oCardInfo.id.replace(/-/g, "")
  )[0];

  const iFoilStart = aDocsFetched.filter(
    (oOwnedInfo) => oOwnedInfo.sId === oCardInfo.id.replace(/-/g, "")
  )[0];

  console.log("iRegStart: ", iRegStart);
  // console.log("iFoilStart: ", iFoilStart);

  const [iRegAmnt, fnSetRegAmnt] = useState(
    !iRegStart ? 0 : iRegStart.iRegular
  );
  const [iFoilAmnt, fnSetFoilAmnt] = useState(
    !iFoilStart ? 0 : iFoilStart.iFoil
  );

  console.log("state: ", iRegAmnt);
